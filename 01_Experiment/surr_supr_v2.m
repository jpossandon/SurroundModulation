...
testType = questdlg('Select task type:', ...
	'Surround supression EEG', ...
	'Experiment','Exp. with filters','Test Trials','Experiment');
...
elseif strcmp(testType,'Experiment')
    ...
    if strcmp(testType,'Exp. with filters')
        fileMAT             = sprintf('SSv2_S%03d%s.mat',str2num(answer{1}),'_wFilt');
    else
        fileMAT             = sprintf('SSv2_S%03d%s.mat',str2num(answer{1}),'');
    end
    if exist(fullfile(resultPATH,fileMAT))
      ...
      switch selection
          ...
          case 'Save as new'
              if strcmp(testType,'Exp. with filters')
                    fileMAT = sprintf('SSv2_S%03d%s%s.mat',str2num(answer{1}),'_wFilt',datestr(now,'HH_MM')); 
              else
                    fileMAT = sprintf('SSv2_S%03d%s.mat',str2num(answer{1}),datestr(now,'HH_MM')); 
...
Screen('Preference', 'SkipSyncTests', 1); % change to 0
...
% GRATING PARAMETERS
PARAMS.GRT.radius               = 2/2*PARAMS.setup.pixels_per_degree; % diameter 2 visual degrees
...
PARAMS.TRIAL.grt_contrasts      = {'20'}; % 50,100
PARAMS.TRIAL.bkg_contrasts      = {'0' '10' '30'}; % 0,(50?)100
PARAMS.TRIAL.bkg_sizes          = {'.25','.5','1'};% additional radius of bakground beyond the radius of the center grating
...
postrials(end+1,:)  = postrials(ismember(postrials(:,3),'0'),:); %repeat none none
