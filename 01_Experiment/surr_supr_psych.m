...
EXP_LOCATION = 1; % 1 - LVPEI 2 - Hamburg 3 - JPO laptop
...
testType = questdlg(sprintf('Select task type:\n''Low vision'' and ''Test Trials'' start at high contrast. ''Test trials'' does not save an ouput.'), ...
	'Surround supression 2AFC', ...
	'Low Vision','Normal vision','Test Trials','Normal vision');
...
PARAMS.TRIAL.trial_per_block     = 25;
PARAMS.TRIAL.trial_per_condition = 50; % makeit a multiple of trial_per_block
...
PARAMS.TRIAL.bkg_contrasts      = {'0','100'};%{'0' '100'}; % 0,(50?)100
PARAMS.TRIAL.bkg_sizes          = {'2','full'};%{'2','4','full'};% size of bakground. full is the complete screen, otherwise number n specify a radial background thath extend (n-1)*GRT.radius beyond the grating outermos edge 
...
if ~exist('CONDpsych','var') && ~exist('TRIAL','var')
    ...
    CONDpsych          = struct('grt_position',postrials(:,1),...
        'bkg_contrast',postrials(:,2), 'bkg_size',postrials(:,3), 'bkg_orient',postrials(:,4),'RT',[]);
...
WaitSecs(2)
flagscape =0;  
for trl = 1:length(CONDpsych)*PARAMS.TRIAL.trial_per_condition
% for trl = 1:length(TRIAL)
    if trl>1 & rem(trl,PARAMS.TRIAL.trial_per_block)==1
        Screen('FillRect',win,127); 
        DrawFormattedText(win, sprintf('Block %d/%d finished\n\n (Press any key to start)',floor(trl/PARAMS.TRIAL.trial_per_block),floor(length(CONDpsych)*PARAMS.TRIAL.trial_per_condition/PARAMS.TRIAL.trial_per_block)),'center','center');
        Screen('FillRect', win, generateVPixxTrigger(0,true) , [-5 -5 2 2]);
        Screen('Flip', win);
        WaitSecs(1)
        KbWait([], 2);
...
if trl>length(CONDpsych)*PARAMS.TRIAL.trial_per_condition/2 % only plot if at least half of the trials were done
    ...
    col = [.894 .102 .11;.215 .494 .721;.302 .686 .29;.596 .305 .639;1 .498 0;1 1 .2;.65 .33 .156; .968 .505 .749];
    for qq = 1:length(CONDpsych)
        ...
        text(.7,qq/10,sprintf('%s %% bkg %s %s\nthreshold:%1.3f (sd:%1.3f)',CONDpsych(qq).bkg_contrast,CONDpsych(qq).bkg_size,CONDpsych(qq).bkg_orient,10.^QuestMean(CONDpsych(qq).q),10.^QuestSd(CONDpsych(qq).q)),'Color',col(qq,:))
    end
